...
# Reproducible Build Script for MT7925 High-Rate Passive RXV Telemetry Ring Buffer on Ubuntu 7.0.0-28
CAN_SRC="/tmp/canonical_mt76_source/drivers/net/wireless/mediatek/mt76"
BUILD_DIR="/tmp/canonical_rxv_highrate_build"
...
echo "=== STEP 2: APPLYING HIGH-RATE RXV TELEMETRY & DIAGNOSTIC COUNTERS PATCH ==="
cat << 'PYEOF' > /tmp/apply_highrate_patch.py
with open("/tmp/canonical_rxv_highrate_build/mt7925/init.c", "r") as f:
    text = f.read()

ring_impl = """
#define RXV_RING_SIZE 32768
...
void mt7925_count_rx_pkt(void)
{
	atomic_inc(&g_rxv_ring.rx_pkts);
}
EXPORT_SYMBOL_GPL(mt7925_count_rx_pkt);
...
static ssize_t
mt7925_rxv_stats_read(struct file *file, char __user *buf,
		      size_t count, loff_t *ppos)
{
	char kbuf[256];
	int len;
	...
	len = scnprintf(kbuf, sizeof(kbuf),
			"rx_pkts: %d\\nrx_rxv: %d\\ndropped: %d\\nring_head: %d\\nring_tail: %d\\n",
			atomic_read(&g_rxv_ring.rx_pkts),
			atomic_read(&g_rxv_ring.rx_rxv),
			atomic_read(&g_rxv_ring.dropped),
			atomic_read(&g_rxv_ring.head),
			atomic_read(&g_rxv_ring.tail));
...
static const struct file_operations mt7925_rxv_stats_fops = {
	.owner = THIS_MODULE,
	.read = mt7925_rxv_stats_read,
	.open = simple_open,
	.llseek = default_llseek,
};
...
ins_str = '\n\tdev_set_drvdata(&dev->mt76.hw->wiphy->dev, &dev->phy);\n\tdebugfs_create_file("mt7925_rxv_telemetry", 0444, dev->mt76.hw->wiphy->debugfsdir, dev, &mt7925_rxv_telemetry_fops);\n\tdebugfs_create_file("mt7925_rxv_stats", 0444, dev->mt76.hw->wiphy->debugfsdir, dev, &mt7925_rxv_stats_fops);'
text = text[:line_end+2] + ins_str + text[line_end+2:]

with open("/tmp/canonical_rxv_highrate_build/mt7925/init.c", "w") as f:
    f.write(text)

with open("/tmp/canonical_rxv_highrate_build/mt7925/mt7925.h", "r") as f:
    hdr_text = f.read()

hdr_text += "\nvoid mt7925_store_rxv_sample(const __le32 *rxv, const struct mt76_rx_status *status);\nvoid mt7925_count_rx_pkt(void);\n"
with open("/tmp/canonical_rxv_highrate_build/mt7925/mt7925.h", "w") as f:
    f.write(hdr_text)

with open("/tmp/canonical_rxv_highrate_build/mt7925/mac.c", "r") as f:
    mac_text = f.read()

func_start = mac_text.find("mt7925_mac_fill_rx(struct mt792x_dev *dev")
line_end_func = mac_text.find("{\n", func_start)
ins_pkt_count = "\n\tmt7925_count_rx_pkt();"
mac_text = mac_text[:line_end_func+2] + ins_pkt_count + mac_text[line_end_func+2:]
...
with open("/tmp/canonical_rxv_highrate_build/mt7925/mac.c", "w") as f:
    f.write(mac_text)

print("[+] Successfully applied High-Rate RXV Telemetry & Diagnostic Counters patch")
PYEOF

python3 /tmp/apply_highrate_patch.py

echo "=== STEP 3: COMPILING HIGH-RATE CANONICAL MT7925 TELEMETRY MODULES ==="
cd "$BUILD_DIR/mt7925"
